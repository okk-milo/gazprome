...
function parseCallHistory(value: unknown): CallHistoryPage { const page = readRecord(value, 'Некорректная страница проверок'); return { items: readArray(page.items, 'Некорректный список проверок').map((value) => { const item = readRecord(value, 'Некорректная проверка'); return { id: readString(item.id, 'Некорректный ID проверки'), fileName: readString(item.fileName, 'Некорректное имя файла'), state: readEnum(item.state, ['upload_pending', 'uploaded', 'transcribing', 'analysing', 'completed', 'no_speech', 'failed'] as const, 'Некорректное состояние'), progress: readNumber(item.progress, 'Некорректный прогресс'), score: item.score === null ? null : readNumber(item.score, 'Некорректная оценка риска'), dealTitle: readString(item.dealTitle, 'Некорректная сделка'), employeeName: readString(item.employeeName, 'Некорректный сотрудник'), createdAt: readString(item.createdAt, 'Некорректная дата') } }), total: readNumber(page.total, 'Некорректное число проверок'), page: readNumber(page.page, 'Некорректный номер страницы'), pageSize: readNumber(page.pageSize, 'Некорректный размер страницы') } }
